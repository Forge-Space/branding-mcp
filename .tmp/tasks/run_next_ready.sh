...
MANIFEST="$ROOT/.tmp/tasks/NEXT_READY.json"
if [[ ! -f "$MANIFEST" ]]; then
...
if [[ -z "$WAVE" ]]; then
	echo "No ready task with mapped wave found."
	echo "See: $ROOT/.tmp/tasks/NEXT_READY.md"
	exit 0
fi
...
if [[ "$CLAIM" == "true" ]]; then
	...
	python3 "$ROOT/.tmp/tasks/planning_doctor.py" --strict
	if ! CLAIM_MANIFEST_PATH="$ROOT/.tmp/tasks/NEXT_READY.json" python3 "$ROOT/.tmp/tasks/claim_next_ready_task.py"; then
