...
function pickRegistryEntry(results, server) {
  return (
    ...
    results.servers?.find(
      (item) =>
        normalizeRepoUrl(item.server?.repository?.url) === normalizeRepoUrl(server.repository?.url)
    ) ??
    null
  );
}
...
async function main() {
  ...
  const publishStatus =
    registryEntry?._meta?.['io.modelcontextprotocol.registry/official']?.status ?? null;
  const report = {
  ...
  report.actionItems = buildActionItems(
    pkg,
    report.npm.latestVersion,
    registryVersion,
    registryEntry
  );
